...
ちなみに or_reduce とは与えられたビットすべてのor(論理和)をとります。したがって、ビットのどれかが1の場合に1になります。or_redeuce の not(否定)はビットの全てが0の場合に1になります。
...
このアルゴリズムは、実は入力データのビット数が2048ビットを超えると、 Vivado の論理合成が終了しませんでした。どうやらこのアルゴリズムは 　Vivado は不得意なようで、最適化に苦労してるようです。しかしビット数が小さい場合は最適化が効くらしく、それなりの結果を出しています。
